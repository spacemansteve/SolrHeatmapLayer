...
Heatmap.Layer = OpenLayers.Class(OpenLayers.Layer, {
  ...
  moveTo: function(bounds, zoomChanged, dragging) {
    ...
    for (var i in this.points) {
      ...
      var intz = src.intensity || this.defaultIntensity;
      ...
      if (!this.cache[intz][rad]) {
        var grd = ctx.createRadialGradient(rad, rad, 0, rad, rad, rad);
        grd.addColorStop(0.0, 'rgba(0, 0, 0, ' + intz + ')');
        grd.addColorStop(1.0, 'transparent');
        this.cache[intz][rad] = grd;
      }

      ctx.fillStyle = this.cache[intz][rad];
      ctx.translate(x, y);
